         ...
         <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="(item , index) in tasklist">
                ...
                <td>
                  <span v-if="item.permission == 0 && item.user_id == authId">
                    <button class="btn btn-primary btn-sm" 
                   
                       @click="handleEdit(item.id)"> Edit
                    </button>
    ...
    data () {
      ...
      return {
          ...
          methods:{
            ...
            handleTaskadd(){
               $('#addTask').modal('hide');
               axios.post('/user-taskadd',this.addTaskData)
                .then((response) => {
                  this.tableData();
            ...
            tableData(nextPage = null) {
              let url = `/user-alltasks?per_page=${this.perPage}`;
          ...
          this.$confirm('This will permanently delete the file. Continue?', 'Warning', {
              ...
              axios.delete('/user-task-delete/'+ id)
              this.$message({
                type: 'success',
       ...
       handleUpdate(id){
         ...
         axios.post('/user-task-update/'+id , this.editTask)
          .then((response) => {
            this.tableData()
          })
          .catch((error) => {
